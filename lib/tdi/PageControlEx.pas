      ...
      for I := 0 to TOwnerAccess(Owner).Tabs.Count - 1 do
        ...
        begin
          if I = FHotTabIndex then
            DoDrawTab(StyleServices.GetElementDetails(ttTabItemHot), I, False)
          else
...
var
  ...
  var
    DoIt: Boolean;
  begin
    if not TOwnerAccess(Owner).OwnerDraw then
      Exit;
    ...
    if DoIt then
      InvalidateRect(Owner.Handle, @FHotTabRect, False);
  end;
